...
const Image = ({ imageName, description }) => {
    ...
    return (
        ...
        <div className="relative flex items-center justify-center h-auto w-full shadow-xl shadow-gray-400 rounded-xl group hover:bg-gradient-to-r from-gray-200 to-[#618c43]" onClick={handleClick}>
            {/* Image element with hover effect */}
            <img className="object-cover rounded-xl group-hover:opacity-25 h-72" src={`images/flora-fauna/${imageName}`} alt=" " />
            ...
            {showFullScreen && (
                <div className="fixed top-0 left-0 w-full h-full flex items-center justify-center z-50 bg-black bg-opacity-50" onClick={handleClose}>
                    <div className="relative w-3/4 h-3/4">
                        {/* Full-size image in the full-screen view */}
                        <img src={`images/flora-fauna/${imageName}`} alt=" " className="w-full h-full object-cover rounded-xl" />
                        ...
                        <button className="absolute top-0 right-0 m-4 p-2 bg-white rounded-full shadow-lg text-xl cursor-pointer">X</button>
                    </div>
                </div>
            )}
        </div>
